...
import { REST_MESSAGE, TIPS_MESSAGE, SETTING_MESSAGE } from "./tray"
...
//         } else {
//           return fs.readFile(
//             path.resolve(__dirname, "..", "renderer" + req.url),
//             (err, data) => {
//               if (err) return
...
export function createRestWindow(): BrowserWindow[] {
  return screen.getAllDisplays().map((display, i) => {
    ...
    win.loadURL(
      createUrl(
        MainEnv.isDev,
        `/LockPage?type=${REST_MESSAGE}${i === 0 ? "&isMainScreen=1" : ""}`
      )
    )
    return win
  })
...
export function createTipsWindow(rect: Rectangle, s: string): BrowserWindow {
  ...
  // 并且为你的应用加载index.html
  win.loadURL(createUrl(MainEnv.isDev, `/Tips?type=${TIPS_MESSAGE}&s=${s}`))
  return win
}
